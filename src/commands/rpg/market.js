...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (sub === 'list') {
            const listings = await db.queryAll('SELECT * FROM rpg_market ORDER BY created_at DESC LIMIT 15');
            ...
            listings.forEach(l => {
                ...
                if (l.is_auction) {
                    ...
                    info = `🔨 **Đấu Giá** | Thầu cao nhất: 🪙 **${(l.current_bid || 0).toLocaleString()}**\nKết thúc trong: ${timeLeft} phút`;
                }
        ...
        if (sub === 'sell') {
            ...
            if (isNaN(price) || price <= 0) return interaction.reply({ content: '❌ Giá không hợp lệ!', flags: 64 });
            ...
            await db.execute(
                'INSERT INTO rpg_market (seller_id, item_id, price, is_auction, expire_at, current_bid) VALUES ($1, $2, $3, $4, $5, $6)',
        ...
        if (sub === 'buy') {
            ...
            const l = await db.queryOne('SELECT * FROM rpg_market WHERE listing_id = $1', [lid]);
            ...
            await db.execute('DELETE FROM rpg_market WHERE listing_id = $1', [lid]);
        ...
        if (sub === 'bid') {
            ...
            await db.execute('UPDATE rpg_market SET current_bid = $1, highest_bidder_id = $2 WHERE listing_id = $3', [bidAmt, userId, lid]);
